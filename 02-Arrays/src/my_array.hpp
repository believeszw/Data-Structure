...
public:
  ...
  friend std::ostream &operator<<(std::ostream &out, MyArray<T> &obj) {
    out << "MyArray size = " << obj.size_ << ", Capacity = " << obj.capacity_
        << std::endl;
    out << "MyArray: [";
...
template <typename T> MyArray<T>::MyArray(const MyArray<T> &arr) {
...
template <typename T> MyArray<T>::MyArray(MyArray<T> &&arr) noexcept {
  assert(this != &arr);
  capacity_ = std::exchange(arr.capacity_, 0);
  size_ = std::exchange(
      arr.size_,
      0); // 非类类型成员的显式移动 // 类类型成员的显式移动使用 std::move
  data_ = std::exchange(arr.data_, nullptr);
  std::cout << "调用  MyArray(const MyArray &&arr) 移动构造函数" << std::endl;
}

template <typename T> MyArray<T> &MyArray<T>::operator=(const MyArray<T> &arr) {
  assert(this != &arr);
...
template <typename T> bool MyArray<T>::IsFull() const {
  return size_ == capacity_;
}

template <typename T> bool MyArray<T>::IsEmpty() const { return size_ == 0; }

template <typename T> void MyArray<T>::Add(int index, T t) {
  if (index < 0 || index > size_) {
...
template <typename T> void MyArray<T>::AddFirst(T t) { Add(0, t); }

template <typename T> void MyArray<T>::AddLast(T t) { Add(size_, t); }

template <typename T> void MyArray<T>::Set(int index, T t) {
  if (index < 0 || index >= size_) {
...
template <typename T> T MyArray<T>::Get(int index) const {
  if (index < 0 || index >= size_) {
...
template <typename T> T MyArray<T>::GetFirst() const { return Get(0); }

template <typename T> T MyArray<T>::GetLast() const { return Get(size_ - 1); }

template <typename T> int MyArray<T>::GetSize() const { return size_; }

template <typename T> int MyArray<T>::GetCapacity() const { return capacity_; }

template <typename T> T MyArray<T>::Remove(int index) {
  if (index < 0 || index >= size_) {
...
template <typename T> T MyArray<T>::RemoveFirst() { return Remove(0); }

template <typename T> T MyArray<T>::RemoveLast() { return Remove(size_ - 1); }

template <typename T> void MyArray<T>::RemoveElement(T t) {
  int index = Find(t);
  index != -1 && Remove(index);
}

template <typename T> void MyArray<T>::Clear() { size_ = 0; }

template <typename T> int MyArray<T>::Find(T t) const {
  for (int i = 0; i < size_; ++i) {
    if (data_[i] == t) {
      return i;
    }
...
template <typename T> bool MyArray<T>::Contain(T t) const {
  return Find(t) != -1;
}

template <typename T> void MyArray<T>::Resize(int new_capacity) {
