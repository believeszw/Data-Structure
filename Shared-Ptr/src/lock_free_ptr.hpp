...
template <typename T> class lf_atomic_shared_ptr {
public:
  ...
  lf_atomic_shared_ptr(const lf_atomic_shared_ptr &other)
  ...
  lf_atomic_shared_ptr(lf_atomic_shared_ptr &&other) noexcept
  ...
  std::shared_ptr<T> lock() const {
    ...
    while (ptr && ref_count->load(std::memory_order_relaxed)) {
      ...
      if (ref_count->compare_exchange_weak(expected, expected + 1,
...
template <typename T> class LockFreeSharedPtr {
public:
  LockFreeSharedPtr() { count_.store(new CountedPtr(0, new T())); }

  explicit LockFreeSharedPtr(T *ptr) { count_.store(new CountedPtr(1, ptr)); }
  ...
  ~LockFreeSharedPtr() { RemoveRef(count_); }

  T *Get() const { return count_.load()->ptr; }

  T &operator*() const { return *(count_.load()->ptr); }

  T *operator->() const { return count_.load()->ptr; }

  operator bool() const { return (count_.load()->ptr != nullptr); }
